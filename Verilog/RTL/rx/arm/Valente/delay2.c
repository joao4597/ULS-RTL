...
int main(int argc, char **argv) {
	...
	uint32_t N_ZEROS, WL_S_L, last_counter[4]= {0}, amplitude_1=0, amplitude_2=0, WL_S_L_A=48, WL_S_L_B=52;			
	int32_t delay[4]={0};
	float decision_delay=MAX_DELAY, true_delay, angle;
	double r, aux;
	FILE *file;
	char str[128], aux_str[256], NMEA[256], *time_p, time[10], number[10], name_file[15];
	int n_gps=0;
	...
	write_values(CONFIG_N_ZEROS, 8); 				// 0 for default value, number of zero to valid detection
	...
	while(1){	
		...
		if(file_counter<4){
			...
			if(true_delay!=MAX_DELAY){
				...
				fprintf(file, "%5.2f  ", angle );
				printf("angle: %5.2f  ",angle);fflush(stdout);
